...
Now that we have the metrics we will use in the Model chapter, we will combine them into a single data frame to make the modelling as straightforward as possible. We also make a plot fo each metric.
...
mp_plot <- ggplot(all_data_comb, aes(x = overall, y = mean_ps)) + 
  geom_point()
mg_plot <- ggplot(all_data_comb, aes(x = overall, y = mean_gp)) + 
  geom_point()
sr_plot <- ggplot(all_data_comb, aes(x = overall, y = suc_rate)) + 
  geom_point()
map_plot <- ggplot(all_data_comb, aes(x = overall, y = mean_adj_ps)) + 
  geom_point()

(mp_plot + mg_plot) / (sr_plot + map_plot)
```

These plots all have the same general shape, though they are all on different scales. In the next chapter we will fit a model to this data.
