...
Now that we have the metrics we will use in the Model chapter, we will combine them into a single data frame to make the modelling as straightforward as possible. We also make a plot of the average values by pick each metric in Figure 6.5.1, note that no points can be directly on top of each other because all points have different `overall` values, but the plot of sis once again jittered vertically. Note that the following two Stack Overflow posts were particularly helpful when writing this code:
