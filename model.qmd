...
Note that the `value` and `pick` functions we implemented in R are not perfect inverses of each other because of the use of `round`, but for the purposes of our R Shiny app this will not be an issue. As one final check, we see what the difference in values was for a [recent NHL trade in which the Philadelpha Flyers acquired the 12th overall selection from the Pittsburgh Penguins in exchange for the 22nd and 31st selections](https://www.sportsnet.ca/nhl/article/penguins-trade-12th-overall-pick-to-flyers-for-nos-22-and-31/). We can see that this difference is about 39 points, which is equivalent to Pittsburgh receiving a mid 5th round pick in surplus value. This seems about right, it is generally accepted that the team that acquires the highest pick almost always gives up more than they receive, which is why teams only trade up if they really like a player who is still available.
...
We now proceed to the Communication chapter, which contains the key results from this report. Note that the steps given on DevOps for Data Science were in Python, I found equivalent steps in R [here](https://vetiver.posit.co/get-started/deploy.html).
