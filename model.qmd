...
To make the rest of this report and shiny app simpler, we will refit the model with scaled PS values. We also will need the scaled predicted values later, so we recalculate those.

```{r}
scal_ps <- data.frame(overall, ps = all_data_adj$ps * C_m[[2]])

nls_scal_ps <- nls(ps ~ SSlogis(log(overall), phi1, phi2, phi3), 
                   data = scal_ps)

summary(nls_scal_ps)

picks <- data.frame(overall = seq(1,224))
scaled_pred <- data.frame(picks, points = predict(nls_scal_ps, picks))
```
...
Note that the `value` and `pick` functions we implemented in R are not perfect inverses of each other because of the use of `round`, but for the purposes of our R Shiny app this will not be an issue. As one final check, we see what the difference in values was for a [recent NHL trade in which the Philadelpha Flyers acquired the 12th overall selection from the Pittsburgh Penguins in exchange for the 22nd and 31st selections](https://www.sportsnet.ca/nhl/article/penguins-trade-12th-overall-pick-to-flyers-for-nos-22-and-31/). We can see that this difference is about 39 points, which is equivalent to Pittsburgh receiving a mid 5th round pick in surplus value. This seems about right, it is widely accepted that the team that acquires the highest pick almost always gives up more than they receive. This is why teams only trade up if they really like a player who is available at the current pick, but that the team thinks will not be available at their next pick. As mentioned in the introduction, our goal is not to evaluate trades based on the "market rate" of picks (ie what they historically get traded for), but this is a good way to confirm that our model is not completely off base from actual NHL trades.
...
We now store the model so we can access it from within our Shiny app. Note that some of the steps given on DevOps for Data Science were in Python, I found the equivalent R steps [here](https://vetiver.posit.co/get-started/deploy.html).
...
s3write_using(scal_ps, FUN = write_parquet, 
              bucket = bucket, object = "scal_ps.parquet")
s3write_using(scaled_pred, FUN = write_parquet, 
              bucket = bucket, object = "pred_vals.parquet")
...
[Here](images/vetiver_api_ss.png) is a screenshot of the popup that opens when the above code is run. We now proceed to the Communication chapter, which contains the key results from this report.
