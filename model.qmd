...
board <- board_s3(bucket = "trevor-stat468") #, region = "us-east-2")

v <- vetiver_model(lm_test, "lm_test")
vetiver_pin_write(board, v)
```

We can also interface with the model using `vetiver_api`. Note I have set `eval = FALSE` for this chunk because the report won't render otherwise, there is a screenshot of the popup below:
...
Here is a screenshot of the popup that opens when the above code is run:

\![Running vetiver_api with our model](images/vetiver_api_ss.png)
