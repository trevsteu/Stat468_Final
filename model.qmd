...
We now store the model and data in an S3 bucket so we can access them from within our Shiny app. Note that some of the steps given on DevOps for Data Science were in Python, I found the equivalent R steps [here](https://vetiver.posit.co/get-started/deploy.html).
...
s3write_using(data.frame(overall = all_data_adj$overall, 
                            ps = scal_ps), FUN = write_parquet, 
              bucket = bucket, object = "scal_ps.parquet")
s3write_using(data.frame(overall = seq(1,224), 
                         pts = value(seq(1,224))), 
              FUN = write_parquet, bucket = bucket, object = "pred_vals.parquet")
