...
We now store the model and data in an S3 bucket so we can access them from within our Shiny app. Note that some of the steps given on DevOps for Data Science were in Python, I found the equivalent R steps [here](https://vetiver.posit.co/get-started/deploy.html). Since `vetiver` cannot be used for `nls` objects, we will use it on the logistic model using success rate to show that I know how to do it. There will then be 2 versions of the app, one using the logistic model and one using the nls model.
...
We can also interface with the model using `vetiver_api`. Note I have set `eval = FALSE` for this chunk because the report won't render otherwise, I've included a screenshot of the popup [here](images/vetiver_api_ss.png) instead.
...
We also run the following to create the files necessary to build a Dockerfile image.
