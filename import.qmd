...
Having considered this, we make the somewhat arbitrary decision to use the 25 drafts between and 1996 and 2020 (inclusive). Note that a significant portion of the players in our dataset are still active, so we will have to adjust for this, which will be done in the [Transform chapter](https://trevsteu.github.io/Stat468_Final/transform.html).
...
We compare the first 10 rows of the 1996 draft table shown above with the table on [Hockey Reference](https://www.hockey-reference.com/draft/NHL_1996_entry.html), and it seems that the function we created does what we want it to do. We can now proceed to the Tidy step.
