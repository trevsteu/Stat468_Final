...
The lessons to take from this plot when creating a model are that it will be necessary to fit a smooth curve to the model (so that $v_i > v_{i+c}$ where $c \in \mathbb Z^+$, and $v_i$ is the value of pick $i$), and that the estimated value of pick $n$ should probably be influenced by the historical value of pick $n$ AND the historical value of picks "close" to $n$. With this in mind, we proceed to the Transform step.
