...
Indeed, both of these are very right-skewed, and clearly a lot of players end up playing a small number of games and are thus not generating able to generate much PS. Finally,
...
-   We will need to fit a curve or line to the data, since if we simply set $v_i$ to be the mean GP or PS of players selected at pick $i$ then we will fail the first fundamental requiremnt, that $v_i > v_{i+k}$ for all $i, k \in \mathbb Z^+$.
