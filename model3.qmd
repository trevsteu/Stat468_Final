...
For each linear model, we generate the fitted values and plot them on top of the historical average values. The averaged values are used to make the plot easier to read, but the lines were fit using the raw values.
...
lm_pred_vals <- data.frame(overall = seq(1, 224), 
                          ...
                          p_reg = exp(logist_pred_vals) / (1 + exp(logist_pred_vals)))
...
for(i in seq(1,3)){
  assign(str_glue("nls_{metrics[i]}"), 
             nls(i + 5 ~ SSlogis(log(overall), phi1, phi2, phi3), data = all_data_adj))
}


nls_ps <- nls(ps ~ SSlogis(log(overall), phi1, phi2, phi3), data = all_data_adj)

nls_pred <- data.frame(pick = all_data_adj$overall, pred_ps = predict(nls_ps, all_data$overall))


ggplot(all_data_adj, aes(x = overall, y = ps)) + 
  geom_point() + 
  geom_line(aes(x = nls_pred$pick, y = nls_pred$pred_ps), col = "red", lwd = 2)
```

```{r}
for(i in seq(1,4)){
  assign(str_glue("nls_{metrics[i]}"), 
         nls(all_data_adj[[i+1]] ~ SSlogis(log(overall), phi1, phi2, phi3)))
}
...
These curves all seem to fit well and all seem to generally describe the data, so we seem to have found a correct function to model the data. We can also plot the curves on top of each other since they are on (roughly) the same scale, we make the green line a little bit thicker so it is easier to see. Note that the scales may not be exactly equal because the predicted value of the first pick may not be exactly 1000 points. When we pick a model we will rescale all the values so that this is the case but for the sake of comparing models this is good enough.

```{r}
ggplot(nls_pred_vals, aes(x = overall)) + 
  geom_line(aes(y = mean_ps), col = "limegreen", lwd = 1.9, lty = 1) + 
  geom_line(aes(y = mean_gp), col = "dodgerblue", lwd = 0.85, lty = 2) +
  geom_line(aes(y = suc_rate), col = "salmon", lwd = 0.85, lty = 3) +
  geom_line(aes(y = mean_adj_ps), col = "purple", lwd = 0.85, lty = 6)
```
...
```{r}
suc_rate <- glm(reg ~ overall, family = "binomial", data = all_data_adj)

overall <- all_data$overall
pred <- predict(suc_rate, data.frame(pick = overall))

prob <- exp(pred) / (1 + exp(pred))

plot(x = overall, y = prob)
```
...
```{r}
C_m <- c(1000 / all_data_adj[1, 5:8])

all_data_scaled <- cbind("overall" = all_data_adj[1], 
                         all_data_adj[-(1:4)] * all_data_adj[-(1:4)] * C_m)

reactable(round(all_data_scaled, 3)) # confirm it worked
```
