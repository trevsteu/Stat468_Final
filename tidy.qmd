...
-   Change `gp` and `ps` values to `0` for players who never played in the NHL or have a negative `ps`. Players who generate a negative `ps` in their career are not necessarily worse picks than those who generate 0 in their careers, so we will just set them to 0. This matches what @moreau_valuation_2025 did.
...
```{r}
all_data_test <- read.csv("all_data.csv")
```
...
```{r, messages = FALSE, warning = FALSE, results= 'hide', eval = FALSE}
con <- dbConnect(duckdb())
